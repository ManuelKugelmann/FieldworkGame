...
- **Find the strongest bot by head-to-head AVG VP.** Assign a *random strategy variant per seat per match* (the `BOTCFG.variant` hook) and rank variants by mean VP — this averages out seat/role so it measures pure bot strength. Winner here: **`hold`** (forage until you can make a ≥3-prestige combo, then publish) beat greedy and biome-steering by ~1.4 VP. An expected-value forager (find value ÷ travel cost) is the *right idea* but **oscillates** (3.7 VP, ~0% wins) if it re-picks its target every step — a committed target is essential.
