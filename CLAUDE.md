...
## Balancing & fairness methodology (hard-won — read before tuning balance)
- **Bots are TEST COVERAGE, not a balance knob.** Make the bot play as *optimally* as you can (maximise its own final score); never tune `botAction` to make the game "look" fair. A weak/greedy bot **hides** ruleset flaws (it doesn't exploit them); a strong bot **exposes** them. Every balance fix goes in the **rules** (`game.ts`), never in the bot.
- **Measure fairness by AVG VP, not win rate.** Win rate magnifies tiny score edges (a ~1 VP mean edge → ~10pp win-rate gap) and is high-variance run-to-run — it *will* send you chasing noise. Primary metric = **mean final score per seat and per role** (fair ≈ all within ~1 VP / a few %). Win rate / winner-minus-loser spread is only for blowout/snowball checks. (We "fixed" seats to a 4.6pt win-spread that was a lucky 350-match sample; the truth was avg VP already within 0.8.)
- **Find the strongest bot by head-to-head AVG VP.** Assign a *random strategy variant per seat per match* (the `BOTCFG.variant` hook) and rank variants by mean VP — this averages out seat/role so it measures pure bot strength. Winner: **`hold`** (forage until you can make a ≥3-prestige combo, then publish) ≈ 10.9 VP, ahead of `biome` 10.1, `ev` 9.5, `greedy` 9.4. All four are viable for *mixed-play* coverage; `hold` is the default for fairness measurement (same strongest bot in every seat → seat diffs are pure ruleset).
- **Heuristic-bot trap — never value speculation over known targets.** The `ev` forager started at a useless 3.6 VP: it *estimated* unrevealed tiles' yield, so it chased far phantom-value tiles that arrived empty and **wandered** (a trace showed 1611 moves / 11 catalogues). Fix: rank only **revealed** finds by value (catalogue-odds × combo-progress ÷ move cost); explore un-entered tiles by **proximity**, never by estimate → 9.5 VP. (Also: a value-myopic forager that *skips* hard-to-catalogue cards starves itself of the high-value combos — you must still collect/attempt them.) When a bot under-scores wildly, **trace its action histogram** (move/catalogue/publish counts) — wandering shows up instantly.
- **Bot cleverness can MANUFACTURE imbalance.** Steering forage toward your discipline's rich biome wrecked role balance (zoo 45% / arch 14%) because biome abundance is wildly uneven (jungle/water huge, only 3 ruins) — it concentrated each specialist on an unequal resource. Plain nearest-find forage (with a value-weighted per-tile catalogue pick) is balance-neutral and was correct.
- **Diagnose the mechanism before fixing.** Split **field-vs-lab VP**, and measure per-seat **catalogues / publishes / round-starts / avg-prestige-per-publish / pool size**. The seat flaw was precise: the field ran a non-multiple-of-N round count, so the **last seat started one fewer round** (7 vs 8) — a structural bug, found by data, not vibes. (Red herrings ruled out along the way: lab order, base car, forage count, publish value — all even.)
- **Structural fixes that landed it (deterministic, no per-match randomisation — see Guardrails):** (1) field ends only on a **whole rotation** (`roundsDone % numPlayers === 0`) so every seat starts equally; (2) a **round-1 AP ramp** (integer 4p `2/3/3/4` by play order) countering the *irreducible* first-mover edge (round 0 must start with someone). Don't over-correct — a full handicap flips the advantage to the last seat. The lab-order / `labInverse` lever became a red herring once the ramp existed.
- **Stop when avg VP is tight.** Final fair state: avg VP within ~**0.8 VP across seats, ~0.35 across roles**. Knobs live in `BAL` (`wander`, `round0Ramp`, `labInverse`, `seasonBenign`, `baseCar`); `BOTCFG.variant` is a measurement harness (ships all-base) — not a game rule.
