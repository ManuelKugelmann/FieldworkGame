...
import { enumerate, botAction, GEAR_MAX } from '../game';
import type { GState } from '../game';
import {
  PLAYER_COLOR, drawBoard, fitCanvas, tileAt, spatialTargets,
  actionLabel, describeTile, sampleChips, maskedChips, gearChips, emptySlots, logToasts, prettyLog, type Action, type Toast,
} from '../render';
            ...
            {Object.entries(G.players).map(([id, p]) => {
              ...
              return (
                <div key={id} className={id === ctx.currentPlayer ? 'pcard cur' : 'pcard'}>
                  <span className="who" style={{ color: PLAYER_COLOR[+id % 4] }}>P{id}</span>{driving ? ' 🚗' : ''}{p.boat ? ' 🛶' : ''} · {vp}pts · {p.prestige}P · {p.money}$ · <span dangerouslySetInnerHTML={{ __html: chips(p.samples) + ' ' + gearChips(p.gear) + emptySlots(GEAR_MAX - p.gear.length) }} /> · pub {p.published.length}
                </div>
              );
            })}
          </div>
