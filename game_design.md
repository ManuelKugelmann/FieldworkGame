...
- **Catalogue (1 AP + roll):** **2d6 + gear + specialist bonus ≥ difficulty**. Difficulty by colour — 🟪 purple **5** (~83% bare) · ⬜ grey **8** (~42%) · 🟦 navy **13** (impossible bare — needs gear/specialist). Tiers: **≥ DC → collected** into your hand; **a failing pair (doubles below the DC) → botched** — the find flees (fauna) / is destroyed, the *only* way an attempt loses it; **any other miss → stays** (try again). Bonuses help twice: they clear the DC and push doubles over it (bare navy botches on any doubles, 1-in-6/attempt).
